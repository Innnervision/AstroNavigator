...
/// Backend integration service for Astro Navigator
/// Handles all backend communication replacing Firebase and Nakama
class BackendIntegrationService {
  ...
  static const String _gameName = 'Astro Navigator';
  static const String _backendUrl = 'http://localhost:8000';

  /// Initialize the backend client
  Future<void> initialize() async {
    try {
      final config = UniverseBackendConfig(
        apiUrl: _backendUrl,
        enableLogging: true,
      );
      ...
      _gameService = GameService(
        repository: GameRepository(client: _client!),
      );
      
      final achievementService = AchievementService(
        repository: AchievementRepository(client: _client!),
      );
      final leaderboardService = LeaderboardService(
        repository: LeaderboardRepository(client: _client!),
      );
      ...
      _logger.i('Backend integration service initialized for Astro Navigator');
    } catch (e) {
  ...
  Future<GameSession?> startSession({Map<String, dynamic>? deviceInfo}) async {
    try {
      ...
      final game = games.results.firstWhere(
        (g) => g.name == _gameName,
  ...
  Future<GameProgress?> updateProgress({
    int? level,
    ...
    try {
      ...
      return await _gameHelper!.updateProgress(
        game.id,
        level: level,
  ...
  Future<GameProgress?> getMyProgress() async {
    try {
      final games = await _gameService!.listGames();
      final game = games.results.firstWhere(
        (g) => g.name == _gameName,
      ...
      return await _gameHelper!.getMyProgress(game.id);
    } catch (e) {
